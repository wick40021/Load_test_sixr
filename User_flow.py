...
# ---------------------------------------
# Generate Random Gameplay Score Payload
# ---------------------------------------
def generate_random_scores(player_id):
    """
    Generates a payload that matches the exact server schema for gameplay score updates.
    """
    possible_results = ["0", "1", "2", "3", "4", "6", "W"]
    overs = 2  # 2 overs = 12 balls

    # Create random RUNS matrix (2 overs x 6 balls)
    ...
    for over in runs:
        for result in over:
            # Collect wickets
            if result == "W":
                wickets.append([ball_index, "BOWLED"])  # ✅ ball_index is an int, not a string
            # Collect sixes (correct format: [ball_index, "6"])
            elif result == "6":
                sixes.append([ball_index, "6"])  # ✅ fixed SIX format
            ball_index += 1
...
class UserFlow(SequentialTaskSet):
    ...
    @task
    def login(self):
        """Step 1: Guest login to get access token"""
    ...
    def gameplay_join(self):
        """Step 5: Join a new gameplay match"""
        url = "/api/v1/gameplay/join"
        headers = {"Authorization": f"Bearer {self.token}"}
        self.matchKey = uuid.uuid4().hex[:32]  # generate random matchKey
    ...
    def update_score(self):
        """Step 6: Submit valid random score (final step)"""
        ...
        payload = {
            ...
            "targetScore": 14  # ✅ FIXED: required in TargetMode!
        }
